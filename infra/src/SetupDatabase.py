...
def process_text_file(filepath):
    ...
    try:
        if filepath.endswith(".md"):
            ...
            # For Word documents (.docx), extract text using python-docx (paragraphs + tables + OCR images)
            ...
            # Tables (flatten each row)
            for table in doc.tables:
                for row in table.rows:
                    cells = [cell.text.strip() for cell in row.cells if cell.text.strip()]
                    if cells:
                        parts.append(" | ".join(cells))
            # Images (OCR) — extract images from the docx package and run Tesseract
            try:
                        ...
                        if name.startswith("word/media/"):
                            try:
                                ...
                                ocr_text = pytesseract.image_to_string(
                                    img, config="--psm 6", lang="fra+eng"
                                )
                                if ocr_text.strip():
                                    parts.append(ocr_text)
                            except Exception as img_err:
                                logger.warning(f"⚠️ OCR image extraction failed for {name}: {img_err}")
            except Exception as zip_err:
                logger.warning(f"⚠️ Unable to open docx as zip for OCR images: {zip_err}")
